...
import { Eye, Trash2, CheckCircle, MoreHorizontal, Loader2, Clock } from "lucide-react";
import {
...
import type { TransferenciaItemWithSolicitacao } from "@/types";

interface TransferenciasTableProps {
  data: TransferenciaItemWithSolicitacao[];
  isLoading: boolean;
  ...
  onRefetch: () => void;
  getSla: (origem: string, destino: string) => number | null;
}

export function TransferenciasTable({
  data, isLoading, isPlanejamento, total, page, totalPages,
  onPageChange, onView, onItemStatus, onRefetch, getSla,
  ...
  async function handleDelete(solicitacaoId: string) {
    ...
    try {
      const res = await fetch(`/api/transferencias/${solicitacaoId}`, { method: "DELETE" });
        ...
        <Table>
          <TableHeader>
            <TableRow style={{ backgroundColor: "#F5F5F5" }}>
              ...
              <TableHead className="font-semibold" style={{ color: "#16455C" }}>Supridor</TableHead>
          ...
          <TableBody>
            {data.length === 0 ? (
              ...
              data.map((item, idx) => {
                ...
                const sla = getSla(item.origem, item.destino);
                const supridor = item.produto?.supridor ?? "-";
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell className="text-sm text-gray-600 max-w-[120px]">
                      <span className="truncate block" title={supridor}>
                        {supridor === "-" ? <span className="text-gray-300">—</span> : supridor}
                      </span>
                    </TableCell>

                    {/* ── Origem ── */}
                    <TableCell className="text-sm text-gray-600">{item.origem}</TableCell>

                    {/* ── Destino ── */}
                    <TableCell className="text-sm text-gray-600">{item.destino}</TableCell>
                    ...
                    <TableCell className="text-center">
                      {sla !== null ? (
                        <Badge
                          variant="outline"
                          className="text-[11px] px-1.5 py-0 gap-1 font-mono"
                    ...
                    {isPlanejamento && (
                      <TableCell className="text-sm text-gray-600">{item.solicitacao.user.nome}</TableCell>
                    )}
